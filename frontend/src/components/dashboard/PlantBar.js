...
export default function PlantBar({ data }) {
  if (!data?.length) return null;
  const chartData = data.filter(d => d.ordered > 0).map(d => ({
    name: `Plant ${d.plant}`,
  ...
  return (
    <div className="border border-zinc-200 bg-white p-5" data-testid="plant-bar-panel">
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="text-sm font-semibold tracking-tight" style={{ fontFamily: 'var(--font-heading)' }}>Plant-wise Ordered vs Delivered</div>
          <div className="text-[10px] text-zinc-400 mt-0.5">All plants — Ordered with GST vs Delivered with GST</div>
        </div>
      ...
      <div style={{ height: 220 }} data-testid="plant-bar-chart">
        <ResponsiveContainer>
          <BarChart data={chartData} barGap={1}>
            ...
            <Bar dataKey="Delivered GST" fill="#1A7A4A" radius={0} />
